  ...
  try {
      ...
      select: {
        ...
        dogs: {
          select: {
            ...
            alteredDate: true,
            emergencyAgent: true,
            emergencyAgentRelationship: true,
            emergencyAgentPhone: true,
            emergencyVetCostLimit: true,
            goodWith: true,
            vaccinations: {
              select: {
                id: true,
    ...
    res.status(200).json({
      ...
      dogs: customer.dogs.map((d) => ({
        ...
        // Sprint 5a enrichment fields
        vet_name: d.vetName,
        vet_phone: d.vetPhone,
        vet_address: d.vetAddress,
        vet_email: d.vetEmail,
        microchip_number: d.microchipNumber,
        color: d.color,
        feeding_method: d.feedingMethod,
        food_type: d.foodType,
        feeding_notes: d.feedingNotes,
        altered_status: d.alteredStatus,
        altered_date: d.alteredDate ? d.alteredDate.toISOString().split('T')[0] : null,
        emergency_agent: d.emergencyAgent,
        emergency_agent_relationship: d.emergencyAgentRelationship,
        emergency_agent_phone: d.emergencyAgentPhone,
        emergency_vet_cost_limit: d.emergencyVetCostLimit,
        good_with: d.goodWith,
        vaccinations: d.vaccinations.map((v) => ({
          id: v.id,
          name: v.name,
          date_given: v.dateGiven.toISOString().split('T')[0],
          expires_at: v.expiresAt ? v.expiresAt.toISOString().split('T')[0] : null,
          verified: v.verified,
        })),
      })),
